...
def get_embeddings_and_neighbors(sentence):
    ...
    try:
        ...
        # If database connection fails, return mock results
        st.warning(f"Database connection failed: {e}. Using mock data.")
        # Mock result: id, llm_generated_idea, text, embedding, score
        mock_html = "<html><body><h1>Mock Webpage</h1><p>This is a mock result because the database is not available.</p></body></html>"
    ...
    if subscription_key:
        with st.spinner("Transcribing..."):
            ...
            if results:
                ...
                subprocess.run(["start", "generated_app.html"], shell=True)  # Use 'start' to open in default browser
